...
- Добавлены common tests для GPX parser/serializer, use case импорта и summary
  импортированного трека.
...
Добавить первый вертикальный срез редактирования трека и экспорта GPX:

- реализовать в `shared` use case для обрезки трека по началу/концу;
- пересчитывать summary для измененного `GpxDocument`;
- добавить простой UI предпросмотра изменений на Android и iOS;
- экспортировать или шарить новый GPX-файл через платформенные механизмы.

После этого можно расширять редактирование: удаление отдельных выбросов, split/merge
сегментов, сглаживание высоты и переименование трека.
